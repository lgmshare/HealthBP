...
androidJunkCode {
    variantConfig {
        create("release") {
            ...
            packageBase = "well.health.longlive.bravoe"  //生成java类根包名
            packageCount = 36 //生成包数量
            activityCountPerPackage = 5 //每个包下生成Activity类数量
            excludeActivityJavaFile = false
            //是否排除生成Activity的Java文件,默认false(layout和写入AndroidManifest.xml还会执行)，主要用于处理类似神策全埋点编译过慢问题
            otherCountPerPackage = 36  //每个包下生成其它类的数量
            methodCountPerClass = 32  //每个类下生成方法数量
            resPrefix = "health_"  //生成的layout、drawable、string等资源名前缀
            drawableCount = 222  //生成drawable资源数量
            stringCount = 209  //生成string数量
        }
    }
}

android {
    namespace = "com.obody.health"
    compileSdk = 34

    defaultConfig {
        applicationId = "com.obody.health"
        minSdk = 24
        targetSdk = 33
        versionCode = 1
        versionName = "1.0.0"
